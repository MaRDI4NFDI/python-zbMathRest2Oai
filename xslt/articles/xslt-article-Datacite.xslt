...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
            ...
            <creators>
                ...
                <xsl:variable name="validTextNodes" select="root/editorial_contributions/text[normalize-space() != '']"/>
        <xsl:if test="$validTextNodes">
            <descriptions>
                <!-- Apply templates to text nodes that contain valid content -->
                <xsl:apply-templates select="$validTextNodes"/>
            </descriptions>
        </xsl:if>
...
<!-- Template for processing text node into description text with its subproperty -->
    ...
    <!-- Only create <description> if the text node contains valid content -->
   ...
   <xsl:template match="msc">
    <!-- Only create the <subject> element if the msc node and text node are valid -->
    <xsl:if test="normalize-space(.) != '' and normalize-space(.) != 'none' and
                  normalize-space(text) != '' and normalize-space(text) != 'none'">
        <subject>
            <!-- Handle subjectScheme -->
            <xsl:attribute name="subjectScheme">
                <xsl:choose>
                    <xsl:when test="normalize-space(scheme) = '' or normalize-space(scheme) = 'none'">
                        <xsl:value-of select="'msc2020'"/>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <!-- Conditionally include classificationCode if code node is valid -->
            <xsl:if test="normalize-space(code) != '' and normalize-space(code) != 'none'">
                <xsl:attribute name="classificationCode">
                    <xsl:value-of select="code"/>
                </xsl:attribute>
            </xsl:if>

            <!-- Always include the text node content -->
